...
merged into a single output. All manifests must share the same namespace. This
supports per-type manifest files (e.g. containers.yaml, routes.yaml) that each
define a single resource type within the same namespace.
