...
8. **App code change required** — `Radius.Compute/containers` injects a JSON blob via `CONNECTION_MYSQLDB_PROPERTIES`, not individual vars. The app's `src/persistence/index.js` must be updated to parse this JSON instead of reading `MYSQL_HOST`, `MYSQL_USER`, etc. individually. See [connection-conventions.md](connection-conventions.md) for helper code.